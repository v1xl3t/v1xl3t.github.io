...
(function () {
  ...
  /* ============================================================
     ...
     Lives in net-core.js now, shared with Hearts. Reversi seats
     exactly two, so it holds one guest and a second knock is turned
     away rather than allowed to overwrite a game in progress.

     The tag keeps the code space separate from the other games, so
     a Reversi code typed into Hearts finds nothing rather than
     finding the wrong table.
     ============================================================ */
  var Net = window.HPNet.create({ tag: 'hpgo1-', max: 1 });
